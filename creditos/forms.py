...
# --- FORMULARIO 1: Para la solicitud pública del Aspirante ---
class SolicitudPublicaForm(forms.ModelForm):
    """
    Formulario público para que cualquier persona solicite un crédito.
    Incluye email y teléfono para contacto y registro posterior.
    """
    class Meta:
        ...
        fields = [
            ...
            'email_aspirante', 'telefono_aspirante',
        ]
        widgets = {
        ...
        labels = {
            'email_aspirante': 'Correo Electrónico',
            'telefono_aspirante': 'Teléfono de Contacto',
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            ...
            if field_name == 'email_aspirante':
                field.widget.attrs.update({'placeholder': 'correo@ejemplo.com'})
            if field_name == 'telefono_aspirante':
                field.widget.attrs.update({'placeholder': '3001234567'})

    def clean_cedula(self):
        """Verificar que no exista una solicitud activa con esta cédula."""
        ...
        if SolicitudCredito.objects.filter(cedula=cedula).exclude(estado__in=estados_inactivos).exists():
            raise forms.ValidationError("Ya existe una solicitud activa con esta cédula.")
        return cedula
...
# --- FORMULARIO PARA REGISTRO DE CREDENCIALES DEL ASPIRANTE ---
class AspiranteRegistroForm(forms.Form):
    """
    Formulario para que el aspirante cree sus credenciales de acceso
    después de pasar el motor de decisión inicial.
    """
...
# --- FORMULARIO 2: Para que el Aspirante suba documentos iniciales ---
class DocumentoForm(forms.ModelForm):
    """
    Formulario para que el Aspirante suba los documentos iniciales.
    """
    class Meta:
        model = Documento
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # Filtramos las opciones para mostrar solo las del aspirante
        opciones_aspirante = next(
            (grupo[1] for grupo in Documento.DOCUMENTOS_CHOICES if grupo[0] == 'Documentos del Aspirante'),
            []
        )
        self.fields['nombre_documento'].choices = opciones_aspirante
...
class DocumentoFinalForm(forms.ModelForm):
    """
    Formulario para que el Aspirante suba los documentos finales de cierre.
    """
    class Meta:
        model = Documento
        fields = ['nombre_documento', 'archivo']
...
class HistorialFiltroForm(forms.Form):
    ...
    # Filtro por analista (el aspirante ya no es un usuario interno)
...
class AspiranteHistorialFiltroForm(forms.Form):
    """
    Un formulario para que el aspirante filtre su historial de estados.
    (Nota: El aspirante típicamente tiene una sola solicitud,
    pero este formulario se mantiene por consistencia)
    """
    ESTADOS_CHOICES = [('', 'Todos los Estados')] + SolicitudCredito.ESTADOS_CHOICES
    estado = forms.ChoiceField(
        label="Estado",
